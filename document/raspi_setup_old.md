# 注意： 現在は（株）アフレル様よりRaspbianを用いた環境構築方法が提供されています。セットアップ時の外部ディスプレイなどが不要となり、提供されるスクリプトによって環境構築が自動化されるため、Raspbianを用いた環境構築を推奨します。ここではUbuntu Mateを使った環境構築方法を紹介しますが、Raspbianを使った環境でもできることは同じです。
